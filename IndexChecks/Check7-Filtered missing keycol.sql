...
SELECT 'Check7 - Filtered index that may not be used by QO' AS [Info],
       ...
       CASE WHEN Tab1.is_column_part_of_key_or_included = 0 THEN 'Warning - SQL Server will not use the index when the IS NULL predicate filtering column is not a key column in the index, or included column in the filtered index definition.' ELSE 'OK' END AS Comment,
