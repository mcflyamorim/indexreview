/* 
  Check 6 - Xtp AvgChain Empty Buckets
*/
...
BEGIN
    ...
    BEGIN
        SELECT 'Check 6 - Xtp AvgChain Empty Buckets' AS [Info],
               ...
               avg_chain_length,
               max_chain_length,
               '[WARNING: Some databases have high avg chain lenght (>5) and high empty buckets count (>50 pct). Verify if there are many rows with duplicate index key values or there is a skew in the key values]' AS [Comment]
        FROM #tmpXIS
        WHERE FLOOR((CAST(empty_bucket_count AS FLOAT) / total_bucket_count) * 100) > 50
              AND [avg_chain_length] > 5
