...
BEGIN
    ...
    (
        SELECT COUNT(*)
        FROM #tmpXNCIS
        WHERE FLOOR(   (CAST(page_update_retry_count AS FLOAT) / CASE
                                                                     WHEN page_update_count = 0 THEN
                                                                         1
                                                                     ELSE
                                                                         page_update_count
                                                                 END
              ...
              OR FLOOR(   (CAST(page_consolidation_retry_count AS FLOAT) / CASE
                                                                               WHEN page_consolidation_count = 0 THEN
                                                                                   1
                                                                               ELSE
              ...
              OR FLOOR(   (CAST(page_split_retry_count AS FLOAT) / CASE
                                                                       WHEN page_split_count = 0 THEN
                                                                           1
                                                                       ELSE
                                                                           page_split_count
                                                                   END
              ...
              OR FLOOR(   (CAST(key_split_retry_count AS FLOAT) / CASE
                                                                      WHEN key_split_count = 0 THEN
                                                                          1
                                                                      ELSE
                                                                          key_split_count
                                                                  END
              ...
              OR FLOOR(   (CAST(page_merge_retry_count AS FLOAT) / CASE
                                                                       WHEN page_merge_count = 0 THEN
                                                                           1
                                                                       ELSE
                                                                           page_merge_count
                                                                   END
              ...
              OR FLOOR(   (CAST(key_merge_retry_count AS FLOAT) / CASE
                                                                      WHEN key_merge_count = 0 THEN
                                                                          1
                                                                      ELSE
                                                                          key_merge_count
                                                                  END
    ...
    BEGIN
        SELECT 'Check 5 - Xtp RangeIX Health' AS [Info],
               ...
               FLOOR(   (CAST(page_update_retry_count AS FLOAT) / CASE
                                                                      WHEN page_update_count = 0 THEN
                                                                          1
                                                                      ELSE
                                                                          page_update_count
                    ...
                    ) AS [page_update_retry_pct_of_total],
               page_consolidation_count,
               page_consolidation_retry_count,
               FLOOR(   (CAST(page_consolidation_retry_count AS FLOAT) / CASE
                                                                             WHEN page_consolidation_count = 0 THEN
                                                                                 1
                                                                             ELSE
                                                                                 page_consolidation_count
                    ...
                    ) AS [page_consolidation_retry_pct_of_total],
               page_split_count,
               page_split_retry_count,
               FLOOR(   (CAST(page_split_retry_count AS FLOAT) / CASE
                                                                     WHEN page_split_count = 0 THEN
                                                                         1
                                                                     ELSE
                                                                         page_split_count
                    ...
                    ) AS [page_split_retry_pct_of_total],
               key_split_count,
               key_split_retry_count,
               FLOOR(   (CAST(key_split_retry_count AS FLOAT) / CASE
                                                                    WHEN key_split_count = 0 THEN
                                                                        1
                                                                    ELSE
                                                                        key_split_count
                    ...
                    ) AS [key_split_retry_pct_of_total],
               page_merge_count,
               page_merge_retry_count,
               FLOOR(   (CAST(page_merge_retry_count AS FLOAT) / CASE
                                                                     WHEN page_merge_count = 0 THEN
                                                                         1
                                                                     ELSE
                                                                         page_merge_count
                    ...
                    ) AS [page_merge_retry_pct_of_total],
               key_merge_count,
               key_merge_retry_count,
               FLOOR(   (CAST(key_merge_retry_count AS FLOAT) / CASE
                                                                    WHEN key_merge_count = 0 THEN
                                                                        1
                                                                    ELSE
                                                                        key_merge_count
                    ...
                    ) AS [key_merge_retry_pct_of_total],
              '[WARNING: Some databases have retry count over 5 percent of total, indicating possible concurrency issues]' AS [Deviation]
        FROM #tmpXNCIS
        WHERE FLOOR(   (CAST(page_update_retry_count AS FLOAT) / CASE
                                                                     WHEN page_update_count = 0 THEN
                                                                         1
