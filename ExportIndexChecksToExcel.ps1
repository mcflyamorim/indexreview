<#
    ...
    PS C:\>& ".\ExportIndexChecksToExcel.ps1" -SQLInstance "DELLFABIANO\SQL2019" -LogFilePath "C:\Temp\IndexReview\" -Force_sp_GetIndexInfo_Execution -CreateTranscriptLog
    .EXAMPLE
    Open a PowerShell console and run the following command:
...
if(-not (Test-Path $LogFilePathQueryPlans))
{
    try {
...
Write-Msg -Message "ShowVerboseMessages: $ShowVerboseMessages"
Write-Msg -Message "------------------------------------------------------------------------"
...
try
{
        ...
        try{
            if (($filename.Name) -like "*Plan cache usage*" ){
                ...
                foreach ($row in $Result)
                {
                    ...
                    if (-not [string]::IsNullOrEmpty($row.statement_plan)) { 
    ...
    [string]$str = "Starting to write Summary results on spreadsheet"
    Write-Msg -Message $str
    ...
    $ws.Cells["A1"].Value = 'SQL Server index review'
    ...
    [string]$str = "Finished to write Summary results on spreadsheet"
    Write-Msg -Message $str
}
catch 
{
    Write-Msg -Message "Error trying to create Summary spreadsheet." -Level Error
    Write-Msg -Message "ErrorMessage: $($_.Exception.Message)" -Level Error
    fnReturn
}
